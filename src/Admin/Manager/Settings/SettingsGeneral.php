...
final class SettingsGeneral {
	...
	public function render( array $values ): void {
		$fields = [
			'root_name' => [
				'label' => __( 'ModPress Root Name', 'modpress' ),
			...
			'root_description' => [
				'label' => __( 'ModPress Description', 'modpress' ),
			...
			'archive_title' => [
				'label' => __( 'ModPress Archive Title', 'modpress' ),
			...
			'archive_description' => [
				'label' => __( 'ModPress Archive Description', 'modpress' ),
			...
			'root_slug' => [
				'label' => __( 'ModPress Root Slug', 'modpress' ),
			...
			'category_slug' => [
				'label' => __( 'Custom Category Slug', 'modpress' ),
			...
			'tag_slug' => [
				'label' => __( 'Custom Tags Slug', 'modpress' ),
				'description' => __( 'The URL slug used for ModPress tags.', 'modpress' ),
				'tooltip' => __( 'Changing this value flushes the WordPress rewrite rules.', 'modpress' ),
				'tooltip_type' => 'info',
			],
			'permalink' => [
				'label' => __( 'ModPress Permalink', 'modpress' ),
			...
			'enable_schema' => [
				'label' => __( 'Enable ModPress Schema', 'modpress' ),
		...
		foreach ( $fields as $key => $field ) {
			...
			$value = 'permalink' === $key
				? PermalinkHelper::sanitize_pattern( $values[ $key ] ?? '' )
				: SanitizationHelper::text( $values[ $key ] ?? $field['default'] ?? '' );
			?>
			<tr>
				...
				<td>
					...
					if ( 'textarea' === ( $field['type'] ?? '' ) ) {
						...
						echo FormFieldHelper::checkbox(
							...
							[
								'id' => $id,
								'checked' => ! empty( $values[ $key ] ?? $field['default'] ),
							]
						);
					} else {
						echo FormFieldHelper::text_input(
							...
							[
								'id' => $id,
								'data-permalink-field' => 'permalink' === $key ? 'permalink' : null,
							]
						);
					}
